...
module.exports = function(){
   ...
   for(let typeButton of typeButtons){
     ...
     this.When(/^I press play button to start the game$/, async function () {        
      ...
      this.When(/^I choose red Bot and yellow as Dum bot$/, async function () {
        let typeButtons = await $('.type-choice-btn' );
        for(let typeButton of typeButtons){
          await typeButton.click();
          let choices = await $('.dropdown-menu.type-choice.show .dropdown-item');
      ...
      this.When(/^again I fill the name for both players$/, async function () {
        await sleep(500 * 2);
       let inputFields = await $('input[placeholder="Namn (2-10 tecken)"]');
       await inputFields[0].sendKeys('BotP1');
       await sleep(500 * 2);
       await inputFields[1].sendKeys('StupidBot1');
       await sleep(500 * 2);
      });

      this.When(/^again I press play button to start the game$/, async function () {
        let clickButton = await $('.begin-btn');
      clickButton.click();
      await sleep(14000);
      });

      this.Then(/^again the two bots should play automatically$/, async function () {
        await sleep(14000);
      });

      this.Then(/^again a message should show who is is the winner$/, async function () {        
        await sleep(sleepTime * 2);
        let messageWin = await driver.findElement(by.css('body > div > main > div > div:nth-of-type(1) > h3 > span'));
        let getmessageWin = await messageWin.getText();
        getmessageWin = getmessageWin.slice(0,10);
        ...
        await sleep(sleepTime * 2);
        assert.equal(getmessageWin, 'BotP1 vann', 'The red BotP1 should win');
        //console.log('Antal vinster för BotP1 är: ' +i)
        await sleep(sleepTime * 2);
      });

      this.Then(/^I verify who is the best player$/, async function () {
        assert.equal(i, 2, 'The Bot player should win 2 times');
      });
